    ...
    if (mapView && miniMapDiv.current) {
      ...
      .then(([MapView, WebMap, Graphic, Extent, Map]) => {
        const webMap = new WebMap({
          portalItem: {
            id: (mapView.view.map as __esri.WebMap)?.portalItem?.id // Utiliza el mismo WebMap que el mapa principal
          }
        })
...
useEffect(() => {
  ...
  if (utilsModule?.logger()) console.log("useEffect =>",{mapView, miniMapDiv})
    ...
    return (
      <div className="w-100 p-1 widget-minimap" style={{ backgroundColor: 'var(--sys-color-primary)', color: 'var(--sys-color-on-primary)' }}>
